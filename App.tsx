...
const App: React.FC = () => {
  ...
  const [isRiskAlertVisible, setIsRiskAlertVisible] = useState(false);
  ...
  const handleSendMessage = useCallback(async (text: string) => {
    ...
    try {
      ...
      for await (const chunk of responseStream) {
        ...
        if (!isRiskDetected && aiResponseText.includes('[RISK_ASSESSMENT_FLAG]')) {
            isRiskDetected = true;
            setIsRiskAlertVisible(true);
        }

        const cleanedText = aiResponseText.replace('[RISK_ASSESSMENT_FLAG]', '').trimStart();

        setMessages(prev => {
            const updated = [...prev];
            updated[updated.length - 1] = { role: Role.AI, text: cleanedText };
            return updated;
        });
      }

      const cleanedFinalText = aiResponseText.replace('[RISK_ASSESSMENT_FLAG]', '').trimStart();
      const finalMessages = [...newMessages, { role: Role.AI, text: cleanedFinalText }];
  ...
  const handleNewChat = () => {
    ...
    setChat(geminiService.initializeChat());
    setIsHistoryPanelOpen(false);
    setIsRiskAlertVisible(false);
  };
    ...
    <div className="flex flex-col h-screen bg-slate-900 text-slate-200 font-sans">
      ...
      <main className="flex-1 overflow-hidden">
        <ChatWindow 
            ...
            isRiskAlertVisible={isRiskAlertVisible}
        />
      </main>
